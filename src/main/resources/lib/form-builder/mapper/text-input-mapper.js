...
exports.map = function(inputConfig) {
    ...
    return InputMapper.map(inputConfig)
        .setValue(inputConfig.input._selected ? inputConfig.input[inputConfig.input._selected].value : null)
        .setPlaceholder(inputConfig.input._selected ? inputConfig.input[inputConfig.input._selected].placeholder : null);
};
